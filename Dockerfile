...
RUN mkdir /apache && cd /apache && git clone https://github.com/apache/cloudstack && cd /apache/cloudstack && LIBS=NONOSS && git clone https://github.com/rhtyd/cloudstack-nonoss.git $LIBS && cd $LIBS && bash -x install-non-oss.sh && rm -rf /apache/cloudstack/NONOSS && sed -i 's/<scope>provided,test<\/scope>/<scope>test<\/scope>/g' /apache/cloudstack/pom.xml && cd /apache/cloudstack && mvn -Pdeveloper -Dsimulator -DskipTests clean install -T3 && cd /apache/cloudstack && pip install --upgrade tools/marvin/dist/Marvin*.tar.gz && cd /apache/cloudstack && mvn -Dsimulator -pl :cloud-client-ui jetty:run -Djava.net.preferIPv4Stack=true &
