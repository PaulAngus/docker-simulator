...
RUN mkdir /acs && cd /tmp && wget http://www.mirrorservice.org/sites/ftp.apache.org/cloudstack/releases/4.14.0.0/apache-cloudstack-4.14.0.0-src.tar.bz2 && tar -jxf /tmp/apache-cloudstack-4.13.0.0-src.tar.bz2 && mv /tmp/apache-cloudstack-4.13.0.0-src/* /acs/ && rm -f /tmp/apache-cloudstack-4.14.0.0-src.tar.bz2 && cd /acs && LIBS=NONOSS && git clone https://github.com/rhtyd/cloudstack-nonoss.git $LIBS && cd $LIBS && bash -x install-non-oss.sh && rm -rf /acs/NONOSS && sed -i 's/<scope>provided,test<\/scope>/<scope>test<\/scope>/g' /acs/pom.xml && cd /acs && mvn -Pdeveloper -Dsimulator -DskipTests clean install -T3 && cd /acs && pip install --upgrade tools/marvin/dist/Marvin*.tar.gz && cd /acs && mvn -Dsimulator -pl :cloud-client-ui jetty:run -Djava.net.preferIPv4Stack=true &
